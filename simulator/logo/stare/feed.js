...
function atomToHtml(uri, element) {
    ...
    var xhr;
    try { if (!xhr && XMLHttpRequest) { xhr = new XMLHttpRequest(); } } catch (e1) { }
    try { if (!xhr && ActiveXObject) { xhr = new ActiveXObject('Msxml2.XMLHTTP'); } } catch (e2) { }
    try { if (!xhr && ActiveXObject) { xhr = new ActiveXObject('Msxml3.XMLHTTP'); } } catch (e3) { }
    try { if (!xhr && ActiveXObject) { xhr = new ActiveXObject('Microsoft.XMLHTTP'); } } catch (e4) { }
    if (!xhr) {
            ...
            if (200 <= xhr.status && xhr.status < 400) {
                ...
                for (var i = 0; i < entries.length; i += 1) {
                    ...
                    try {
                        ...
                        if (window.console && window.console.log) { window.console.log("Error:", e); }
                    }
